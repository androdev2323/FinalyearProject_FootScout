...
class playerrepoImpl(private val dao: playerDao,@ApplicationContext val context: Context):playerrepository {
    override suspend fun insertALL(player: List<Player>) {
        dao.insertALL(player)
    }

    override suspend fun searchplayers(query: String): Flow<NetworkResult<List<Player>>> {

        return flow {
            emit(NetworkResult.loading)
            try {
                val list = dao.searchPlayersbyName(query)
    ...
    override suspend fun importPlayersFromCsvIfNeeded() {
        val playerCount = dao.getPlayerCount()

        // If the database is empty, load data from the CSV
        if (playerCount == 0) {
            context.assets.open("aggregated_2021_2024_90.csv").bufferedReader().use { csvReader ->
                csvReader.readLine() // Skip the header row
                csvReader.lineSequence()
